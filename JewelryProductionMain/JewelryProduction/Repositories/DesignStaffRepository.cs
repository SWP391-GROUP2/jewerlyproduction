...
{
    public class DesignStaffRepository : IDesignStaffRepository
    {
        private readonly JewelryProductionContext _context;
        private readonly IMapper _mapper;

        public DesignStaffRepository(JewelryProductionContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public async Task<List<StaffWithCountDTO>> GetStaffs()
        {
            var users = await _context.UserRoles
                ...
                .Where(result => result.r.Name.Equals("designstaff"))
                .Select(result => new
                {
                    User = _mapper.Map<AppUserDTO>(result.combined.u),
                    OrderCount = _context.Orders
                        .Where(cr => cr.DesignStaffId == result.combined.u.Id)
                        .Count()
                })
                .ToListAsync();
