...
namespace JewelryProduction.Services
{
    public class VnPayService : IVnPayService
    {
        ...
        public VnPayService(IConfiguration config, JewelryProductionContext context)
        {
            _config = config;
            _context = context;
        }
        ...
        public async Task<string> CreatePaymentUrl(HttpContext context, VnPaymentRequestModel model)
        {
            ...
            vnpay.AddRequestData("vnp_Amount", (1000000000).ToString()); //số tiền thanh toán. số tiền không mang các ký tự phân tách thập phân, phần nghìn, ký tự tiền tệ. để gửi số tiền thanh toán là 100,000 vnd (một trăm nghìn vnđ) thì merchant cần nhân thêm 100 lần(khử phần thập phân), sau đó gửi sang vnpay là: 10000000
        ...
        public VnPaymentResponseModel Deposit(IQueryCollection collection)
        {
            var vnPay = new VnPayLibrary();
